...
// GET - Retrieve a specific organization knowledge entry
export const GET = withRateLimit(async (
  ...
  try {
    ...
    const response = await fetch(`${PYTHON_SERVICE_URL}/organization-knowledge/${organizationId}/${id}`, {
      method: 'GET',
    ...
    logServerError('organization-knowledge GET by ID API', error);
    return NextResponse.json(
...
// PUT - Update a specific organization knowledge entry
export const PUT = withRateLimit(async (
  ...
  try {
    ...
    const result = updateOrganizationKnowledgeSchema.safeParse(body);
    ...
    if (!response.ok) {
      ...
      console.error('Python service error:', error);
      return NextResponse.json(
        { error: 'Failed to update organization knowledge', status: response.status },
        { status: response.status || 500 }
      );
    }
    ...
    logServerError('organization-knowledge PUT by ID API', error);
    return NextResponse.json(
      formatApiError(error),
      { status: 500 }
    );
...
// DELETE - Delete a specific organization knowledge entry
export const DELETE = withRateLimit(async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Python service error:', error);
      return NextResponse.json(
        { error: 'Failed to delete organization knowledge', status: response.status },
        { status: response.status || 500 }
      );
    }
    ...
    logServerError('organization-knowledge DELETE by ID API', error);
    return NextResponse.json(
      formatApiError(error),
      { status: 500 }
    );
