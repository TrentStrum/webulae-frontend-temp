...
export const GET = withRateLimit(async (
  ...
  try {
    ...
    const response = await fetch(`${PYTHON_SERVICE_URL}/company-faqs/${faqId}`, {
      method: 'GET',
    ...
    logServerError('company faq GET', error);
...
export const PUT = withRateLimit(async (
  ...
  try {
    ...
    logServerError('company faq PUT', error);
    return NextResponse.json(formatApiError(error), { status: 500 });
  }
});
...
export const DELETE = withRateLimit(async (
  ...
  try {
    ...
    logServerError('company faq DELETE', error);
    return NextResponse.json(formatApiError(error), { status: 500 });
  }
}); 